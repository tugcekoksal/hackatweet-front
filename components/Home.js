...
function Home() {
  ...
  const [hashtags, setHashtags] = useState([]); // État pour les tendances
  ...
  useEffect(() => {
    fetch("http://localhost:3000/tweet")
      ...
      .then((data) => {
        ...
        setHashtags(data.map((tweet, index) => tweet.hashtags));
      });
  }, [tweets]);
  ...
  return (
    <main className="flex justify-between bg-slate-800 h-screen divide-x divide-slate-500 text-white  font-mont-heavy">
      ...
      <div className="w-[45%] flex-grow  divide-y divide-slate-500">
        ...
        <div className="overflow-auto max-h-[400px]">{lastTweets}</div>
      </div>
      <div className="w-[30%]">
        {" "}
